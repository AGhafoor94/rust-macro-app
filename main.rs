...
use std::{ env, fs::{self, DirEntry, File, FileType},  io::{self, Read}, path::Path, process::{ Output }, str };
// use windows::Win32::UI::Input::KeyboardAndMouse::{*};
use windows::Win32::{Foundation::*, UI::{Input::KeyboardAndMouse::{ VkKeyScanW, GMMP_USE_DISPLAY_POINTS }, WindowsAndMessaging::*}};
// use windows::core::{ s };
use windows::Win32::UI::Input::KeyboardAndMouse::{*};
...
fn main() -> Result<(), std::io::Error> {
    ...
    unsafe {
        ...
        */
        let mut window_text:Vec<u16> = Vec::new();
        let mut _current_window:HWND = GetForegroundWindow();
        // let new_chrome_window:HWND = GetForegroundWindow();
        let _ = SetActiveWindow(_current_window);
        let _ = SetFocus(_current_window);
        std::thread::sleep(std::time::Duration::from_millis(1000));
        let return_i32_val:i32 = GetWindowTextW(_current_window, &mut window_text);
        let _window_text_u8_array: &[u16] = &window_text;
        // println!("{:?}", str::from_utf8(&win_text_u16_array).expect("Can't"));
        println!("WINDOW TEXT: {:?}", return_i32_val);
        println!("RETURN I32 VALUE: {:?}", GetWindowTextLengthA(_current_window));
        std::thread::sleep(std::time::Duration::from_millis(2000));
        // PREVIOUSLY USED
        // let length:i32 = GetWindowTextLengthA(_current_window) + 1;
        // let mut window_text: Vec<u16> = vec![];
        // GetWindowTextW(_current_window,&mut window_text);
        ...
        // let _current_window_opened = GetClientRect(_current_window,&mut client_rect);
        // PREVIOUSLY USED ABOVE

        // let _ = SetWindowPos(_current_window,HWND_TOP,0,0,client_rect.right,client_rect.bottom,SWP_SHOWWINDOW);


        // let windows_position = MapWindowPoints(0, _current_window,0 );
        // PREVIOUSLY USED
        // let mut text:Vec<u16> = vec![0;length as usize];
        // let _ = GetWindowTextW(_current_window,&mut text);
        // PREVIOUSLY USED ABOVE
        ...
        let _ = SetCursorPos(focus_rect.left + 100, focus_rect.top + 100);
        ...
        for key in hold_keys_vector_steps.iter() {
            ...
            for _ in 0..key.r#loop {
                ...
                if key.held && key.code < 800{
                ...
                else if key.name == "Command" {
                    
                    let output= execute_command("cmd", &["/C", &key.sentence]);
                    match output {
                        Ok(o) => {
                            let string_match = str::from_utf8(&o.stdout).expect("Can't").split("\n");
                            if str::contains(&key.sentence, ".jnlp") || str::contains(&key.sentence, ".exe") {
                    ...
                    match key.held {
                        true => {
                            ...
                            if !String::is_empty(&key.sentence) {
                                ...
                                if key.code == 801 {
                                    send_mouse_input_message(mouse_coords[0],mouse_coords[1], false, 0x01, key.held)
                                }
                                if key.code == 802 {
                                    // send_input_messages(0x0002, false, true)
                                    
                                    send_mouse_input_message(mouse_coords[0],mouse_coords[1], false, 0x02, key.held)
                                }
                            }else {
                                if key.code == 801 {
                        ...
                        false => {
                            ...
                            if key.name.contains("move") {
                                ...
                                send_mouse_input_message(mouse_coords[0],mouse_coords[1], true, 0, key.held)
                            }
                    ...
                    key.sentence.as_bytes().into_iter().for_each(|f| {
                        ...
                        // println!("f: {}, Key from Json: {}, total key: {}, hex code: {}",f,key_from_json,_u16_total_key,hex_code);
                        // match second_char {
                        //     "A" => _u16_total_key = _u16_total_key + 10,
                        //     "B" => _u16_total_key = _u16_total_key + 11,
                        //     "C" => _u16_total_key = _u16_total_key + 12,
                        //     "D" => _u16_total_key = _u16_total_key + 13,
                        //     "E" => _u16_total_key = _u16_total_key + 14,
                        //     "F" => _u16_total_key = _u16_total_key + 15,
                        //     _ => _u16_total_key = _u16_total_key + second_char.parse::<u16>().unwrap(),
                        // }
                        // let character:u8 = format_hex_code.as_bytes()[format_hex_code.len() - 1];
                        // println!("{:?}",_u16_total_key);
                        // println!("{:?}",hex_code.parse::<u16>().unwrap());
                        
                        // println!("{:?}",*f as u16);
                        // println!("HERE {}, {}, char: {}", (key_from_json >> 8 & 1) == 1, key_from_json,&_key_char);
        ...
        // _mouse_movements.iter().for_each(|f| {
        //     println!("320: HELD: {}, {}", f.name, f.held);
        //     match f.held {
        //         true => {
        ...
        //                 count += 1
        //             };
        //             println!("329: WORD SPLIT: {:?}", mouse_coords);
        //             if f.name.to_lowercase().contains("left") {
        //                 send_mouse_input_message(mouse_coords[0],mouse_coords[1], true, 0x01, true)
        //             }
        //             if f.name.to_lowercase().contains("right") {
        //                 send_input_messages(0x0002, false, true)
        //             }
        //         },
        //         false => {
        ...
        //                     count += 1
        //                 };
        //                 send_mouse_input_message(mouse_coords[0],mouse_coords[1], true, 0, false)
        //             }
        //             std::thread::sleep(std::time::Duration::from_millis(500));
        //             if f.name.to_lowercase().contains("left") {
        //                 if f.r#loop > 1 {
        //                     for i in 0..f.r#loop {
        //                         println!("MOUSE CLICK: {}", i);
        //                         send_mouse_input_message(0,0, false, 0x01, false)
        //                     }
        //                 }else {
        //                     send_mouse_input_message(0,0, false, 0x01, false)
        //                 }
        ...
        //             }
        //             if f.name.to_lowercase().contains("right") {
        //                 send_mouse_input_message(0,0, false, 0x02, false)
        //             }
        //         }
        //     }
        //     // holding_keys_to_release.push(0x0001);
        //     // holding_keys_to_release.push(0x0002);
        // });
        ...
        // let _ = GetWindowRect(_current_window, &mut window_rect);

        // println!("WINDOW RECTANGLE: {:?}", window_rect);


        // send_mouse_input_message(0,0, false, 0x02); 
        // send_mouse_input_message(window_rect.bottom,window_rect.right, true, 0); 
...
fn get_mouse_events() {
    unsafe {
        ...
        let mouse_move_point_struct:MOUSEMOVEPOINT = MOUSEMOVEPOINT {
            x: 0 & 0x0000FFFF,
            y: 0 & 0x0000FFFF,
            time: 64,
            dwExtraInfo: 0x01
        ...
        println!("523: {:?}",&mouse_move_point_struct);
        println!("524: {:?}",&mouse_move_points);
    }
}
fn send_mouse_input_message(x:i32, y:i32, move_mouse:bool, mouse_button: u16, held:bool) {
    ...
    unsafe {
        ...
        let mut point_struct:POINT = POINT { ..Default::default() };
        let _ = GetCursorPos(&mut point_struct);
        let mut _input_mouse_struct: INPUT = INPUT {..Default::default()};
                        ...
                        _input_mouse_struct = INPUT {
                            ...
                            Anonymous: INPUT_0 {
                                mi: MOUSEINPUT {
                                    ...
                                    dy: y * (65535 / GetSystemMetrics(SM_CXSCREEN)),
                                    mouseData: 0x01,
